...
function db_phong_select_all()
{
    if (isset($_POST["seach"])) {
        ...
        $sql = "SELECT * FROM `phong` WHERE id NOT IN(SELECT id_Phong FROM `book` WHERE '$checkin' >= check_in AND check_out >= '$checkin' AND  check_in <= '$checkout' AND check_out >= '$checkout' AND status = 1 OR status = 2 OR status = 3 OR status = 6 OR status = 7) AND slNguoiLon <= $slNguoiLon AND slTreEm <=$treEm";
    } else {
